...
async def list_bebidas(request):
    """
    ...
    if request.method == "GET":
        queryset = await sync_to_async(list)(Bebidas.objects.all())
        ...
        if serializer.is_valid():
            bebida = await sync_to_async(serializer.save)()
...
async def retrieve_bebida(request, pk):
    """
    ...
    """
    try:
        instance = await sync_to_async(Bebidas.objects.get)(pk=pk)
