...
async def list_pedidos(request):
    """
    ...
    if request.method == "GET":
        queryset = await sync_to_async(list)(Pedidos.objects.all())
        ...
        if is_valid:
            pedido = await sync_to_async(serializer.save)()
...
async def retrieve_pedidos(request, pk):
    """
    ...
    """
    try:
        instance = await sync_to_async(Pedidos.objects.get)(pk=pk)
