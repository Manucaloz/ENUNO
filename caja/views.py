...
async def list_caja(request):
    ...
    if request.method == "GET":
        queryset = await sync_to_async(list)(Caja.objects.all())
        ...
        if await sync_to_async(serializer.is_valid)():
            caja = await sync_to_async(serializer.save)()
...
async def retrieve_caja(request, pk):
    """
    ...
    """
    try:
        instance = await sync_to_async(Caja.objects.get)(pk=pk)
